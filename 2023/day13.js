...
function findHorizontalSmudge(puzzle) {
  for (let i = 0; i < puzzle.length - 1; i++) {
    let k = 0;
    let cumulativeErrors = 0;
    while ((i - k) >= 0 && (i + 1 + k) < puzzle.length) {
      let errors = 0;
      for (let j = 0; j < puzzle[i].length; j++) {
        if (puzzle[i - k][j] !== puzzle[i + 1 + k][j]) {
          errors++;
          cumulativeErrors++;
        }
      }
      if (cumulativeErrors > 1) break;
      k++;
    }
    if (i - k < 0 || i + 1 + k === puzzle.length) {
      if (cumulativeErrors === 1) return i + 1;
    }
  }
  return 0;
}

function findVerticalSmudge(puzzle) {
  for (let j = 0; j < puzzle[0].length - 1; j++) {
    let k = 0;
    let cumulativeErrors = 0;
    while (((j - k) >= 0) && ((j + 1 + k) < puzzle[0].length)) {
      for (let i = 0; i < puzzle.length; i++) {
        if (puzzle[i][j - k] !== puzzle[i][j + 1 + k]) cumulativeErrors++;
      }
      if (cumulativeErrors > 1) break;
      k++;
    }
    if (j - k < 0 || j + 1 + k === puzzle[0].length) {
      if (cumulativeErrors === 1) return j + 1;
    }
  }
  return 0;
}
...
function partTwo() {
  ...
  const puzzles = input.split('\n\n');
  const puzzleArrays = puzzles.map(p => p.split('\n'));
  const horizontal = puzzleArrays.map(findHorizontalSmudge);
  const vertical = puzzleArrays.map(findVerticalSmudge);
